...
exports.borrow = async (toolId, userId, quantity, distributorId) => {
  ...
  if (!tool) throw new Error('工具不存在');
  if (!quantity || quantity <= 0) throw new Error('借出数量非法');
  if (tool.stock < quantity) throw new Error('工具库存不足');
  if (tool.status !== 'available') throw new Error('工具不可借用');
...
exports.restore = async (toolId, userId, quantity, collectorId) => {
  const tool = await Tool.findById(toolId);
  if (!tool) throw new Error('工具不存在');
  if (tool.status !== 'borrowed') throw new Error('该工具未被借出');
  if (userId.toString() != tool.borrower.toString()) throw new Error('无法归还，不是借出人');
  if (quantity > tool.usedQuantity) throw new error('归还数量不能大于借出数量');
